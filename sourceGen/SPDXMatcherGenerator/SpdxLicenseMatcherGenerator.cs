    ...
    public class SpdxLicenseMatcherGenerator : IIncrementalGenerator
    {
        ...
        private static IEnumerable<(string SpdxId, string Pattern)> CreatePatternsFromXml(string xmlContent)
        {
            ...
            foreach (XElement license in licenseElements)
            {
                ...
                patternBuilder.Replace("/", "\\/");

                string finalPattern = Regex.Replace(Regex.Replace($"^\\s*{patternBuilder.ToString()}\\s*$", @"(\\s[\+\*])*\\s\+(\\s[\+\*])*", @"\s+"), @"(\\s\*)+", @"\s*");
        ...
        private static void BuildPatternRecursive(XElement element, StringBuilder patternBuilder, XNamespace ns)
        {
            foreach (XNode node in element.Nodes())
            {
                if (node is XText textNode)
                {
                    ...
                    {
                        patternBuilder.Append(Regex.Escape(cleanedText).Replace("\\ ", @"\s+"));
...
static partial class SpdxLicenseMatcher
{
            ...
            sb.Append(@"()
    {
        return (""");
            sb.Append(spdxId);
            sb.Append(@""", new Regex(");
            sb.Append(SymbolDisplay.FormatLiteral(pattern, true));
            sb.Append(@", RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled, TimeSpan.FromSeconds(1)));
    }
}");
            return sb.ToString();
        }
